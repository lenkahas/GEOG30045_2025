...
1. Using the plot above, what is your guess for:
    a. how much *schnitzel* would you guess would be eaten by someone who drank 1 beer? *just eyeballing it, about 130 grams*
    b. how much *schnitzel* would you guess would be eaten by someone who drank 2.5 beers? *just eyeballing it, about 110 grams*
    c. how much *schnitzel* would you guess would be eaten by someone who drank no beer? *to me, visually extending that downward trend it looks like about 50 grams*
    d. how much *beer* would be drank by someone someone who ate 100 grams of schnitzel? *plausible for 3 beers, or for something like half a beer*
    e. how much *beer* would be drank by someone who ate 125 grams of schitzel? *This could be either 1 beer, or something like 2.25 or 3.25 beers*
    c. If you had to explain how you made these guesses, what would you say you did? *I check the value where I’m predicting, and visually “draw” a line through points nearby to see where the relationship might be going. Others may do this differently, though!*
:::
...
1. What is the predicted `schnitzel` value when $k=n$? What does that value represent? *This represents the average schnitzel consumption for people at oktoberfest. This is because the average of the nearest observations is just the average of all the observations!*
2. Why do we see a big jump when `nearness_cutoff`$=15$? *This is a tie. You can see in the preceeding plot, two observations are labelled as 15.5, meaning that they share the 15 and 16 ranks between them. This means that once you go from $k=14$ to $k=15$, you have to pick one of those two observations to use for your average. And, it also means you “jump” two observations in one step of the nearness cutoff.*
:::
...
1. Why do the KNN learners "flatten out" at the very small and very big values of `beer`? *Because there are no new observations out past these values. Once you get to the edge of the point cloud, no new information is provided as a function of getting more extreme. So, the KNN learner just continues to predict the same value.*
2. Which KNN learner do you think has the lowest error? *$k=1$passes through each point exactly, and so clearly has the lowest possible error of zero.*
3. Which KNN learner do you think represents the best tradeoff in terms of the bias-variance tradeoff? *I think that $k=4$ has the best traedoff here. It’s fairly smooth visually, but still manages to follow the point cloud effectively.*
...
Use the plot to approximately judge which model would have the best performance on this validation set. *Again working visually, I look at 1 and follow it up to the value 127. Then, I look at 2 and follow it up to 127 again, and the same for $(3.98$ and $(4,225)$. I personally think visually, it either $k=4$ or $k=12$ would be best. The $k=4$ model seems to be too high for small beer values, while $k=12$ is definitely too high for $3$ and $4$ beers.*
:::
...
1. Fit the $k=1$, $k=4$, $k=12$, and $k=23$ models from the plot above. 
    a. Which one has the best validation error? You can use `yardstick::rsme_vec(prediction, known)`, which is the `rmse_vec()` function from the `yardstick` package, to calculate the RMSE of a vector of predictions and a vector of known values.  *see code chunks below*
    b. Does the model that has the lowest validation error match the one you expected to have the best bias-variance tradeoff? If they differ, why do you think they differ? *so, I thought k=4 would have good performance, and it does. But, k=12 really doesn’t! Note that this is probably because it’s just too smooth. Indeed, when you visually inspect a fitted line, you’re more likely to reason directly in terms of residual lengths (and so in terms of absolute error), while the statistical approaches we compare will sometimes use squared error not absolute error. So, it’s easy to mislead yourself visually, and thus important to play around with the actual loss calculations!*
2. Fit a KNN model to the `Boston` data from ISL, predicting `medv` using `lstat`.
    a. What is the optimum $k$ value? *see chunks below*
    b. What is the predicted `medv` when `lstat` is 5, 10, and 15? *see chunks below*
    c. How do these predictions differ from a linear model's predictions? *see chunks below, predictions for large lstat are higher in the linear model, likely because the model cannot handle the strong downwards curving relationship between medv and lstat.*
    d. Which model do you expect to have a better model fit to *new data*? *Given the nonlinearity, KNN*
...
```{r}
k1 = train(schnitzel ~ beer, data=oktoberfest, method='knn', tuneGrid=data.frame(k=1))
k4 = train(schnitzel ~ beer, data=oktoberfest, method='knn', tuneGrid=data.frame(k=4))
k12 = train(schnitzel ~ beer, data=oktoberfest, method='knn', tuneGrid=data.frame(k=12))
k23 = train(schnitzel ~ beer, data=oktoberfest, method='knn', tuneGrid=data.frame(k=23))
```

```{r}
known_results <- data.frame(
    beer = 1:4,
    schnitzel = c(127, 127, 98, 225)
)

data.frame(
    rmse_train = c(
    yardstick::rmse_vec(predict(k1), oktoberfest$schnitzel),
    yardstick::rmse_vec(predict(k4), oktoberfest$schnitzel),
    yardstick::rmse_vec(predict(k12), oktoberfest$schnitzel),
    yardstick::rmse_vec(predict(k23), oktoberfest$schnitzel)
    ),
    rmse_test = c(
        yardstick::rmse_vec(predict(k1, newdata=known_results), known_results$schnitzel),
        yardstick::rmse_vec(predict(k4, newdata=known_results), known_results$schnitzel),
        yardstick::rmse_vec(predict(k12, newdata=known_results), known_results$schnitzel),
        yardstick::rmse_vec(predict(k23, newdata=known_results), known_results$schnitzel)
    ),
    k=c(1,4,12,23)
) |> arrange(rmse_test) |> knitr::kable()
```

```{r}
## could also do this in a for loop: 

for(k in c(1,4,12,23)){
    model = train(schnitzel ~ beer, data=oktoberfest, method='knn', tuneGrid=data.frame(k=k))
    predy_train = predict(model)
    predy_test = predict(model, newdata=known_results)

    rmse_train = yardstick::rmse_vec(predy_train, oktoberfest$schnitzel)
    rmse_test =  yardstick::rmse_vec(predy_test, known_results$schnitzel)

    print(paste0("k=",k," has rmse: ", round(rmse_test, 4)))
}
```
...
boston_nn <- train(
    ...
    data=Boston,
    method='knn', 
    tuneGrid = data.frame(k=seq(1, 50, 2)) # I am checking 1 to 50 in steps of 2
)

# which row of the results df is the best on RMSE?
best_row = which.min(boston_nn$results$RMSE)

ggplot(boston_nn$results, aes(x=k, y=RMSE)) + 
    geom_line() + 
    geom_point(x=boston_nn$results$k[best_row], y=boston_nn$results$RMSE[best_row], color='red')
```

```{r}
# note that the plot curves upwards, suggesting that we've found a real minimum after 
# which the out of sample error will start to increase. You can check this yourself by 
# increasing the maximum k beyond 50!
```

## 2b

```{r}
# make a new dataframe with the values you want to predict
scenario <- data.frame(lstat=c(5, 10, 15))

# and predict those values!
scenario |> mutate(
    pred_nn = predict(boston_nn, newdata=scenario)
) |> knitr::kable()
```
...
```{r}
# fit an LM now
boston_lm = train(medv ~ lstat, data=Boston, method='lm')

# and do the same thing
scenario |> mutate(
    pred_nn = predict(boston_nn, newdata=scenario),
    pred_lm = predict(boston_lm, newdata=scenario),
) |> knitr::kable()
```
...
1. Fit a KNN model to the `Boston` data from ISL, predicting `medv` using `lstat` and `age`. 
    a. What is the best value of $k$? *eleven*
    b. Now, force the variables to be re-scaled to have a mean of zero and a variance of one using the `preProcess='scale'` option for `caret::train()`. What is the best value of $k$ there? Is the error higher or lower? *thirteen. Error is lower in the scaled model!*
    c. What is the standard deviation of the `lstat` feature? How about the `age` feature? *SD of lstat is about 7.14, while age is up at 28.14*
    d. If a test point is 1 unit away from a training point in model (a), what's the largest difference in `age` between the test point and the training point? How about `lstat`? *if a test point is 1 unit of distance away, it’s either one year older (or younger) or it’s one percentage point different on the lower socioeconomic status population in the area, which is what lstat measures.*
    e. If a test point is 1 unit away from a training point in model (b), what's the largest difference in `age` between the test point and training point? How about `lstat`? *If a test point is 1 unit away from a training point in model (b), what’s the largest difference in age between the test point and training point? How about lstat? Here, now that we’ve standardized the input data, a unit difference in the distance means the house is twenty eight years apart, or it’s seven percentage points different on the lower socioeconomic status population percentage.*
    f. Given what you learned about (d) and (e), is the model from part (a) more sensitive to `age` or `lstat`? How about the model in part (b)? *the model from part a will be more sensitive to changes in age because the variability in age changes more slowly as a function of distance. Put another way, observations are more likely to be close on their lstat feature because its variance/standard deviation is so much smaller than the age feature. This means that an overall distance between two observations in the first model has a small amount “caused” by differences in lstat, but much more “caused” by differences in age. So, two houses that are “similar” will tend to be quite similar in age, but less similar on the lstat feature. In model b, a unit of distance corresponds to a standard deviation change in either input variable, so model b will listen to both of them equally.*
:::
